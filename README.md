# og-packs

The 44 official CESP sound packs from [peon-ping](https://github.com/PeonPing/peon-ping).

Each pack contains an `openpeon.json` manifest (CESP v1.0) and a `sounds/` directory with audio files.

## Packs

| Pack | Character | Franchise | Language |
|---|---|---|---|
| `peon` | Orc Peon | Warcraft III | English |
| `peon_es` | Orc Peon | Warcraft III | Spanish |
| `peon_fr` | Orc Peon | Warcraft III | French |
| `peon_cz` | Orc Peon | Warcraft III | Czech |
| `peon_de` | Orc Peon | Warcraft III | German |
| `peon_pl` | Orc Peon | Warcraft III | Polish |
| `peon_ru` | Orc Peon | Warcraft III | Russian |
| `peasant` | Human Peasant | Warcraft III | English |
| `peasant_cz` | Human Peasant | Warcraft III | Czech |
| `peasant_es` | Human Peasant | Warcraft III | Spanish |
| `peasant_fr` | Human Peasant | Warcraft III | French |
| `peasant_ru` | Human Peasant | Warcraft III | Russian |
| `acolyte_de` | Undead Acolyte | Warcraft III | German |
| `acolyte_ru` | Undead Acolyte | Warcraft III | Russian |
| `brewmaster_ru` | Pandaren Brewmaster | Warcraft III | Russian |
| `murloc` | Murloc | Warcraft III | English |
| `wc2_peasant` | Human Peasant | Warcraft II | English |
| `sc_kerrigan` | Sarah Kerrigan | StarCraft | English |
| `sc_battlecruiser` | Battlecruiser | StarCraft | English |
| `sc_scv` | SCV | StarCraft | English |
| `sc_firebat` | Firebat | StarCraft | English |
| `sc_medic` | Medic | StarCraft | English |
| `sc_tank` | Siege Tank | StarCraft | English |
| `sc_vessel` | Science Vessel | StarCraft | English |
| `sc_terran` | Terran Units Mix | StarCraft | English |
| `ra2_kirov` | Kirov Airship | Red Alert 2 | English |
| `ra2_soviet_engineer` | Soviet Engineer | Red Alert 2 | English |
| `ra_soviet` | Soviet Soldier | Red Alert | English |
| `glados` | GLaDOS | Portal | English |
| `tf2_engineer` | Engineer | Team Fortress 2 | English |
| `dota2_axe` | Axe | Dota 2 | English |
| `duke_nukem` | Duke Nukem | Duke Nukem | English |
| `rick` | Rick Sanchez | Rick and Morty | English |
| `sopranos` | Tony Soprano | The Sopranos | English |
| `hd2_helldiver` | Helldiver | Helldivers 2 | English |
| `molag_bal` | Molag Bal | The Elder Scrolls | English |
| `sheogorath` | Sheogorath | The Elder Scrolls | English |
| `ocarina_of_time` | Navi | The Legend of Zelda | English |
| `aoe2` | Taunts | Age of Empires II | English |
| `aom_greek` | Greek Villager | Age of Mythology | Greek |
| `goblin` | Goblin Merchant | World of Warcraft Classic | English |
| `clean_chimes` | Clean Chimes | (UI sounds) | English |
| `league_of_legends` | League of Legends | League of Legends | English |
| `cs16` | Radio | Counter-Strike 1.6 | English |

## Using these packs

Install [peon-ping](https://github.com/PeonPing/peon-ping) and use any pack:

```bash
peon --pack glados
```

Or browse all packs at [openpeon.com/packs](https://openpeon.com/packs).

## Creating your own pack

**This repo is closed to new pack submissions.** The OG collection is curated by the core team and is not accepting additions.

To create and distribute your own pack, host it in your own repo and submit it to the [PeonPing/registry](https://github.com/PeonPing/registry). Community packs work identically to OG packs — see [openpeon.com/create](https://openpeon.com/create) for the full guide.

## License

Individual packs may have their own licenses (see each pack's `openpeon.json`). The repository structure and tooling are MIT licensed.
